...
class FastByteArrayStreamsTest {
	...
	private void setArray (DataInput r, byte[] array) {
		if (r instanceof BAIS x){
			x.array(array, 0, array.length);
		} else {
	...
	void x (String hex, ThrowingConsumer<DataOutput> write, ThrowingConsumer<DataInput> readAndVerify) {
		...
		try {
			for (DataOutput w : new DataOutput[]{new BAOS(), new OpenDataOutputStream()}){
				for (DataInput r : new DataInput[]{new BAIS(ByteArrays.EMPTY_ARRAY), new OpenDataInputStream()}){
					reset(w);
	...
	@Test
	void testEOF () throws UTFDataFormatException {
		BAIS r = new BAIS(ByteArrays.EMPTY_ARRAY);
	...
	void testWriteFloat () {
		x("00000000 4991a2b0 c7000000 cf000000 46fffe00 4f000000 40490fdb",
			...
			r -> {
				assertEquals(0, r.readFloat());
				assertEquals(0x123456, r.readFloat());
				assertEquals(Short.MIN_VALUE, r.readFloat());

				assertEquals(Integer.MIN_VALUE, r.readFloat());
				assertEquals(Short.MAX_VALUE, r.readFloat());
				assertEquals(Integer.MAX_VALUE, r.readFloat());

				assertEquals(3.14159265f, r.readFloat());
			}
		);
	}
	...
	void testWriteDouble () {
		x("00000000000000004132345600000000c0e0000000000000c1e000000000000040dfffc00000000041dfffffffc00000c3e000000000000043e0000000000000400921fb54442d18",
			...
			r -> {
				assertEquals(0, r.readDouble());
				assertEquals(0x123456, r.readDouble());
				assertEquals(Short.MIN_VALUE, r.readDouble());

				assertEquals(Integer.MIN_VALUE, r.readDouble());
				assertEquals(Short.MAX_VALUE, r.readDouble());
				assertEquals(Integer.MAX_VALUE, r.readDouble());

				assertEquals(Long.MIN_VALUE, r.readDouble());
				assertEquals(Long.MAX_VALUE, r.readDouble());

				assertEquals(Math.PI, r.readDouble());
			}
		);
	}
	...
	@Test
	void testWriteUTF8 () throws UTFDataFormatException {
		String s = "\0\r\nChars in UTF-8 🚀💯!";
	...
	@Test
	void testObjectStreams () throws IOException, ClassNotFoundException {
		BAOS w = new BAOS();
	...
	void testCustomObjects () {
		...
		p.writeExternal(w);
		p.writeExternal(w);
		assertEquals("00044a6f686e0005536d6974680786040f00000020736f6d652072616e646f6d2062797465732061732070686f746f20696d616765".repeat(3), toHexString(w.array(), 0, w.size()));
		assertEquals(w.position, w.length());
		assertTrue(w.length() <= w.array().length);

		var r = new BAIS(w.array(), 0, w.size());
		var p2 = new Person();
